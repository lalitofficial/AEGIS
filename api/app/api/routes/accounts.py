...
def _format_last_scan(last_scan: datetime | None) -> str:
...
async def get_monitored_accounts(db: Session = Depends(get_db)):
    ...
    try:
        accounts = db.query(Account).filter(Account.is_monitored.is_(True)).all()
        ...
        raise HTTPException(status_code=500, detail="Failed to fetch monitored accounts") from e
