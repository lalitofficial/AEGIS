...
async def get_dashboard_metrics(db: Session = Depends(get_db)):
    ...
    try:
        ...
        confirmed_frauds = db.query(FraudAlert).filter(FraudAlert.status == "Blocked").count()

        # Calculate fraud detection rate
        fraud_detection_rate = (
            (confirmed_frauds / total_transactions * 100) if total_transactions > 0 else 0
        )
...
async def get_fraud_trends(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to fetch fraud trends"
        ) from e
...
async def get_fraud_type_distribution(db: Session = Depends(get_db)):
    ...
    try:
        ...
        fraud_types = (
            db.query(FraudAlert.type, func.count(FraudAlert.id).label("count"))
            .group_by(FraudAlert.type)
            .all()
        )
